...
static struct core_timer {
	...
	dahdi_timespec_t start_interval;
...
static inline struct __kernel_old_timespec timespec64_to_timespec(const struct timespec64 ts64)
{
	return *(const struct __kernel_old_timespec *)&ts64;
}
#else
static inline struct __kernel_old_timespec timespec64_to_timespec(const struct timespec64 ts64)
{
	struct __kernel_old_timespec ret;
...
static inline void ktime_get_ts(struct __kernel_old_timespec *ts)
{
...
static unsigned long core_diff_ms(dahdi_timespec_t *t0, dahdi_timespec_t *t1)
{
	long nanosec, sec;
	unsigned long ms;
