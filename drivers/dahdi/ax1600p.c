...
#include <linux/errno.h>   // test, 2012.11.20
#include <linux/pci.h>
...
#include <asm/io.h>
#include "proslic.h"
#include <linux/string.h>  //test, 2012.11.20
...
#include "fxo_modes.h"
#define TDM_REVH_MATCHALL  //test, 2013-0402
...
#define WC_CS		0x2
#define WC_VER		0x3
#define WC_DMACTL	0x5		//test by ly, 
...
static struct wctdm_desc wctdm = { "ATCOM AX1600P/800P", 0 };  //test, 2012.11.20
...
static unsigned int batthigh;		
static unsigned int batoffhook;  
...
{
	volatile unsigned int *writechunk = NULL;
	int x;
	if (ints & 0x08) 
		/* Write is at interrupt address.  Start writing from normal offset */
		{
			if(wc->HW_VER == 0xa8)
				writechunk = wc->writechunk + DAHDI_CHUNKSIZE*3;
			else if(wc->HW_VER == 0xa7)
				writechunk = wc->writechunk + DAHDI_CHUNKSIZE*4;
			else	if(wc->HW_VER < 0xa7)
				writechunk = wc->writechunk + DAHDI_CHUNKSIZE*5;
		}
	else 
	...
	if(wc->HW_VER < 0xA7)
		{
			...
			/* Send a sample, as a 32-bit word */
				...
				writechunk[5*x+3] = 0;
       			...
       			if (wc->cardflag & (1 << 3))
					writechunk[5*x+(wc->sync_chan)%5] |= (wc->chans[3]->writechunk[x] << 24);
				if (wc->cardflag & (1 << 2))
					writechunk[5*x+(wc->sync_chan)%5] |= (wc->chans[2]->writechunk[x] << 16);
				if (wc->cardflag & (1 << 1))
					writechunk[5*x+(wc->sync_chan)%5] |= (wc->chans[1]->writechunk[x] << 8);
				if (wc->cardflag & (1 << 0))
					writechunk[5*x+(wc->sync_chan)%5] |= (wc->chans[0]->writechunk[x]);
          

       			if (wc->cardflag & (1 << 7))
					writechunk[5*x+(1+wc->sync_chan)%5] |= (wc->chans[7]->writechunk[x] << 24);
				if (wc->cardflag & (1 << 6))
					writechunk[5*x+(1+wc->sync_chan)%5] |= (wc->chans[6]->writechunk[x] << 16);
				if (wc->cardflag & (1 << 5))
					writechunk[5*x+(1+wc->sync_chan)%5] |= (wc->chans[5]->writechunk[x] << 8);
				if (wc->cardflag & (1 << 4))
					writechunk[5*x+(1+wc->sync_chan)%5] |= (wc->chans[4]->writechunk[x]);	
		
       			if (wc->cardflag & (1 << 11))
					writechunk[5*x+(2+wc->sync_chan)%5] |= (wc->chans[11]->writechunk[x] << 24);
				if (wc->cardflag & (1 << 10))
					writechunk[5*x+(2+wc->sync_chan)%5] |= (wc->chans[10]->writechunk[x] << 16);
				if (wc->cardflag & (1 << 9))
					writechunk[5*x+(2+wc->sync_chan)%5] |= (wc->chans[9]->writechunk[x] << 8);
				if (wc->cardflag & (1 << 8))
					writechunk[5*x+(2+wc->sync_chan)%5] |= (wc->chans[8]->writechunk[x]);		
	

       			if (wc->cardflag & (1 << 15))
					writechunk[5*x+(3+wc->sync_chan)%5] |= (wc->chans[15]->writechunk[x] << 24);
				if (wc->cardflag & (1 << 14))
					writechunk[5*x+(3+wc->sync_chan)%5] |= (wc->chans[14]->writechunk[x] << 16);
				if (wc->cardflag & (1 << 13))
					writechunk[5*x+(3+wc->sync_chan)%5] |= (wc->chans[13]->writechunk[x] << 8);
				if (wc->cardflag & (1 << 12))
					writechunk[5*x+(3+wc->sync_chan)%5] |= (wc->chans[12]->writechunk[x]);		
		...
		{
			//memset((void *)writechunk,0,DAHDI_MAX_CHUNKSIZE * 2 *(NUM_CARDS+NUM_SYNC));	
			for (x=0;x<DAHDI_CHUNKSIZE;x++) 
				{
					/* Send a sample, as a 32-bit word */
					writechunk[4*x] = 0;
...
#else			
					if (wc->cardflag & (1 << 3))
						writechunk[4*x+3] |= (wc->chans[3]->writechunk[x] << 24);
					if (wc->cardflag & (1 << 2))
						writechunk[4*x+3] |= (wc->chans[2]->writechunk[x] << 16);
					if (wc->cardflag & (1 << 1))
						writechunk[4*x+3] |= (wc->chans[1]->writechunk[x] << 8);
					if (wc->cardflag & (1 << 0))
						writechunk[4*x+3] |= (wc->chans[0]->writechunk[x]);

					if (wc->cardflag & (1 << 7))
						writechunk[4*x] |= (wc->chans[7]->writechunk[x] << 24);
					if (wc->cardflag & (1 << 6))
						writechunk[4*x] |= (wc->chans[6]->writechunk[x] << 16);
					if (wc->cardflag & (1 << 5))
						writechunk[4*x] |= (wc->chans[5]->writechunk[x] << 8);
					if (wc->cardflag & (1 << 4))
						writechunk[4*x] |= (wc->chans[4]->writechunk[x]);	

					if (wc->cardflag & (1 << 11))
						writechunk[4*x+1] |= (wc->chans[11]->writechunk[x] << 24);
					if (wc->cardflag & (1 << 10))
						writechunk[4*x+1] |= (wc->chans[10]->writechunk[x] << 16);
					if (wc->cardflag & (1 << 9))
						writechunk[4*x+1] |= (wc->chans[9]->writechunk[x] << 8);
					if (wc->cardflag & (1 << 8))
						writechunk[4*x+1] |= (wc->chans[8]->writechunk[x]);	

					if (wc->cardflag & (1 << 15))
						writechunk[4*x+2] |= (wc->chans[15]->writechunk[x] << 24);
					if (wc->cardflag & (1 << 14))
						writechunk[4*x+2] |= (wc->chans[14]->writechunk[x] << 16);
					if (wc->cardflag & (1 << 13))
						writechunk[4*x+2] |= (wc->chans[13]->writechunk[x] << 8);
					if (wc->cardflag & (1 << 12))
						writechunk[4*x+2] |= (wc->chans[12]->writechunk[x]);	          
...
#else		
					if (wc->cardflag & (1 << 3))
						writechunk[3*x+(wc->sync_chan)%3] |= (wc->chans[3]->writechunk[x] << 24);
					if (wc->cardflag & (1 << 2))
						writechunk[3*x+(wc->sync_chan)%3] |= (wc->chans[2]->writechunk[x] << 16);
					if (wc->cardflag & (1 << 1))
						writechunk[3*x+(wc->sync_chan)%3] |= (wc->chans[1]->writechunk[x] << 8);
					if (wc->cardflag & (1 << 0))
						writechunk[3*x+(wc->sync_chan)%3] |= (wc->chans[0]->writechunk[x]);
          

       				if (wc->cardflag & (1 << 7))
						writechunk[3*x+(1+wc->sync_chan)%3] |= (wc->chans[7]->writechunk[x] << 24);
					if (wc->cardflag & (1 << 6))
						writechunk[3*x+(1+wc->sync_chan)%3] |= (wc->chans[6]->writechunk[x] << 16);
					if (wc->cardflag & (1 << 5))
						writechunk[3*x+(1+wc->sync_chan)%3] |= (wc->chans[5]->writechunk[x] << 8);
					if (wc->cardflag & (1 << 4))
						writechunk[3*x+(1+wc->sync_chan)%3] |= (wc->chans[4]->writechunk[x]);	
#endif	
#endif
				}
...
#endif
#define WC_SYNCFLAG   0xefc3efc3
static inline void wctdm_receiveprep(struct wctdm *wc, unsigned char ints)
{
	volatile unsigned int *readchunk = NULL;
	int x;

	if (ints & 0x08)
		{
			if(wc->HW_VER < 0xA7)
				readchunk = wc->readchunk + DAHDI_CHUNKSIZE*(NUM_CARDS+NUM_SYNC)/4;
			else if(wc->HW_VER == 0xA7)
				readchunk = wc->readchunk + DAHDI_CHUNKSIZE*(NUM_CARDS/4);
			else if(wc->HW_VER == 0xA8)
				readchunk = wc->readchunk + DAHDI_CHUNKSIZE*3;
		}
	else
	...
	for (x=0;x<DAHDI_CHUNKSIZE;x++) {
...
#else  
			if (wc->cardflag & (1 << 3))
				wc->chans[3]->readchunk[x] = (readchunk[5*x+(1+wc->sync_chan)%5] >> 24) & 0xff;
			if (wc->cardflag & (1 << 2))
				wc->chans[2]->readchunk[x] = (readchunk[5*x+(1+wc->sync_chan)%5] >> 16) & 0xff;
			if (wc->cardflag & (1 << 1))
				wc->chans[1]->readchunk[x] = (readchunk[5*x+(1+wc->sync_chan)%5] >> 8) & 0xff;
			if (wc->cardflag & (1 << 0))
				wc->chans[0]->readchunk[x] = (readchunk[5*x+(1+wc->sync_chan)%5]) & 0xff;


  			if (wc->cardflag & (1 << 7))
				wc->chans[7]->readchunk[x] = (readchunk[5*x+(2+wc->sync_chan)%5] >> 24) & 0xff;
			if (wc->cardflag & (1 << 6))
				wc->chans[6]->readchunk[x] = (readchunk[5*x+(2+wc->sync_chan)%5] >> 16) & 0xff;
			if (wc->cardflag & (1 << 5))
				wc->chans[5]->readchunk[x] = (readchunk[5*x+(2+wc->sync_chan)%5] >> 8) & 0xff;
			if (wc->cardflag & (1 << 4))
				wc->chans[4]->readchunk[x] = (readchunk[5*x+(2+wc->sync_chan)%5]) & 0xff;	

  			if (wc->cardflag & (1 << 11))
				wc->chans[11]->readchunk[x] = (readchunk[5*x+(3+wc->sync_chan)%5] >> 24) & 0xff;
			if (wc->cardflag & (1 << 10))
				wc->chans[10]->readchunk[x] = (readchunk[5*x+(3+wc->sync_chan)%5] >> 16) & 0xff;
			if (wc->cardflag & (1 << 9))
				wc->chans[9]->readchunk[x] = (readchunk[5*x+(3+wc->sync_chan)%5] >> 8) & 0xff;
			if (wc->cardflag & (1 << 8))
				wc->chans[8]->readchunk[x] = (readchunk[5*x+(3+wc->sync_chan)%5]) & 0xff;			 
	
	
  			if (wc->cardflag & (1 << 15))
				wc->chans[15]->readchunk[x] = (readchunk[5*x+(4+wc->sync_chan)%5] >> 24) & 0xff;
			if (wc->cardflag & (1 << 14))
				wc->chans[14]->readchunk[x] = (readchunk[5*x+(4+wc->sync_chan)%5] >> 16) & 0xff;
			if (wc->cardflag & (1 << 13))
				wc->chans[13]->readchunk[x] = (readchunk[5*x+(4+wc->sync_chan)%5] >> 8) & 0xff;
			if (wc->cardflag & (1 << 12))
				wc->chans[12]->readchunk[x] = (readchunk[5*x+(4+wc->sync_chan)%5]) & 0xff;		
	...
	else if(wc->HW_VER == 0xA7)
		{
			...
			for (x=0;x<DAHDI_CHUNKSIZE;x++)
				{
					if (wc->cardflag & (1 << 3))
						wc->chans[3]->readchunk[x] = (readchunk[4*x] >> 24) & 0xff;
					if (wc->cardflag & (1 << 2))
						wc->chans[2]->readchunk[x] = (readchunk[4*x] >> 16) & 0xff;
					if (wc->cardflag & (1 << 1))
						wc->chans[1]->readchunk[x] = (readchunk[4*x] >> 8) & 0xff;
					if (wc->cardflag & (1 << 0))
						wc->chans[0]->readchunk[x] = (readchunk[4*x]) & 0xff;

					if (wc->cardflag & (1 << 7))
						wc->chans[7]->readchunk[x] = (readchunk[4*x+1] >> 24) & 0xff;
					 if(wc->cardflag & (1 << 6))
						wc->chans[6]->readchunk[x] = (readchunk[4*x+1] >> 16) & 0xff;
					if (wc->cardflag & (1 << 5))
						wc->chans[5]->readchunk[x] = (readchunk[4*x+1] >> 8) & 0xff;
					if (wc->cardflag & (1 << 4))
						wc->chans[4]->readchunk[x] = (readchunk[4*x+1])& 0xff;

					if (wc->cardflag & (1 << 11))
						wc->chans[11]->readchunk[x] = (readchunk[4*x+2]>> 24) & 0xff;
					if (wc->cardflag & (1 << 10))
						wc->chans[10]->readchunk[x] = (readchunk[4*x+2] >> 16) & 0xff;
					if (wc->cardflag & (1 << 9))
						wc->chans[9]->readchunk[x] = (readchunk[4*x+2] >> 8) & 0xff;
					if (wc->cardflag & (1 << 8))
						wc->chans[8]->readchunk[x] = (readchunk[4*x+2]) & 0xff;
					
					if (wc->cardflag & (1 << 15))
						wc->chans[15]->readchunk[x] = (readchunk[4*x+3] >> 24) & 0xff;
					if (wc->cardflag & (1 << 14))
						wc->chans[14]->readchunk[x] = (readchunk[4*x+3] >> 16) & 0xff;
					if (wc->cardflag & (1 << 13))
						wc->chans[13]->readchunk[x] = (readchunk[4*x+3] >> 8) & 0xff;
					if (wc->cardflag & (1 << 12))
						wc->chans[12]->readchunk[x] = (readchunk[4*x+3]) & 0xff;	
				}
		}
	...
	else if(wc->HW_VER == 0xA8)
		{
			...
			for (x=0;x<DAHDI_CHUNKSIZE;x++)
				{
					if (wc->cardflag & (1 << 3))
						wc->chans[3]->readchunk[x] = (readchunk[3*x+(1+wc->sync_chan)%3] >> 24) & 0xff;
					if (wc->cardflag & (1 << 2))
						wc->chans[2]->readchunk[x] = (readchunk[3*x+(1+wc->sync_chan)%3] >> 16) & 0xff;
					if (wc->cardflag & (1 << 1))
						wc->chans[1]->readchunk[x] = (readchunk[3*x+(1+wc->sync_chan)%3] >> 8) & 0xff;
					if (wc->cardflag & (1 << 0))
						wc->chans[0]->readchunk[x] = (readchunk[3*x+(1+wc->sync_chan)%3]) & 0xff;
					
					if (wc->cardflag & (1 << 7))
						wc->chans[7]->readchunk[x] = (readchunk[3*x+(2+wc->sync_chan)%3] >> 24) & 0xff;
					if(wc->cardflag & (1 << 6))
						wc->chans[6]->readchunk[x] = (readchunk[3*x+(2+wc->sync_chan)%3] >> 16) & 0xff;
					if (wc->cardflag & (1 << 5))
						wc->chans[5]->readchunk[x] = (readchunk[3*x+(2+wc->sync_chan)%3] >> 8) & 0xff;
					if (wc->cardflag & (1 << 4))
						wc->chans[4]->readchunk[x] = (readchunk[3*x+(2+wc->sync_chan)%3]) & 0xff;
				}
		}
#ifdef AUDIO_RINGCHECK
...
static int __wait_access(struct wctdm *wc, int card)
{
    ...
    origjiffies = jiffies;  // test, 2012.11.20
...
#define MS_PER_CHECK_HOOK 16
  ...
  else	//added by bluce for offhook vol; test 2012.11.20
  	{
  		if((fxo->offhook)&&(abs(b)>batoffhook))
  			{
  				if (fxo->battery == BATTERY_PRESENT)
  					{
  						if (fxo->battdebounce)
...
DAHDI_IRQ_HANDLER(wctdm_interrupt)
{
	...
	//for (x=0;x<4;x++) {
	for (x=0;x<NUM_CARDS;x++) {    //test, 2012.11.20
		if (wc->cardflag & (1 << x) &&
...
static int wctdm_init_voicedaa(struct wctdm *wc, int card, int fast, int manual, int sane)
{
	...
	if(wc->HW_VER < 0xA7)
		{
			wctdm_setreg(wc, card, 34, (3-card%4) * 8+(card/4)*48);
			wctdm_setreg(wc, card, 36, (3-card%4) * 8+(card/4)*48);
		}
	else if(wc->HW_VER == 0xA7)
		{
	...
	//wctdm_setreg(wc, card, 36, (3-card) * 8);  //test: 2013-
...
static int wctdm_init_proslic(struct wctdm *wc, int card, int fast, int manual, int sane)
{
	...
	if(wc->HW_VER <0xA7)
		{
			wctdm_setreg(wc, card, 2, (3-card%4) * 8+(card/4)*48);    // Tx Start count low byte  0
			wctdm_setreg(wc, card, 4, (3-card%4) * 8+(card/4)*48);    // Rx Start count low byte  0			
		}
	else if(wc->HW_VER == 0xa7)		//ax1600P
		{
    ...
    //wctdm_setreg(wc, card, 4, (3-card) * 8);    // Rx Start count low byte  0 ; 2012-11-20
...
static int wctdm_initialize(struct wctdm *wc)
{
	...
	//wc->span.irq = wc->dev->irq;  //2012-11-20
	wc->span.spantype = SPANTYPE_ANALOG_MIXED;
	wc->span.flags = DAHDI_FLAG_RBS;
	//init_waitqueue_head(&wc->span.maintq);  //2012-11-20
...
static int wctdm_hardware_init(struct wctdm *wc)
{
	...
	unsigned char x,y;
	int failed;
	unsigned char port_num = 0;  // test 2012.11.20 
	...
	if(wc->HW_VER == 0xa7)
	...
	else if (wc->HW_VER == 0xa8)
		{
			port_num = 8;
			outl(wc->writedma,                    wc->ioaddr + WC_DMAWS);		/* Write start */
			outl(wc->writedma + DAHDI_CHUNKSIZE * 4 * 3 - 4, wc->ioaddr + WC_DMAWI);		/* Middle (interrupt) */
			outl(wc->writedma + DAHDI_CHUNKSIZE * 8 * 3  - 4, wc->ioaddr + WC_DMAWE);			/* End */
	
			outl(wc->readdma,                    	 wc->ioaddr + WC_DMARS);	/* Read start */
			outl(wc->readdma + DAHDI_CHUNKSIZE * 4 * 3 - 4, 	 wc->ioaddr + WC_DMARI);	/* Middle (interrupt) */
			outl(wc->readdma + DAHDI_CHUNKSIZE * 8 * 3  - 4, wc->ioaddr + WC_DMARE);	/* End */
		}
	else if(wc->HW_VER < 0xa7)
		{
			port_num = 16;	
			outl(wc->writedma,                    wc->ioaddr + WC_DMAWS);		/* Write start */
		outl(wc->writedma + DAHDI_CHUNKSIZE * 4 * ((NUM_CARDS+NUM_SYNC)/4) - 4, wc->ioaddr + WC_DMAWI);		/* Middle (interrupt) */
		outl(wc->writedma + DAHDI_CHUNKSIZE * 8 * ((NUM_CARDS+NUM_SYNC)/4)  - 4, wc->ioaddr + WC_DMAWE);			/* End */
	
		outl(wc->readdma,                    	 wc->ioaddr + WC_DMARS);	/* Read start */
		outl(wc->readdma + DAHDI_CHUNKSIZE * 4 * ((NUM_CARDS+NUM_SYNC)/4) - 4, 	 wc->ioaddr + WC_DMARI);	/* Middle (interrupt) */
		outl(wc->readdma + DAHDI_CHUNKSIZE * 8 * ((NUM_CARDS+NUM_SYNC)/4)  - 4, wc->ioaddr + WC_DMARE);	/* End */
		}
	// end of the code
	...
	//for (x = 0; x < NUM_CARDS; x++) {
	for (x = 0; x < port_num; x++) {   // test 2012.11.20
...
static void wctdm_start_dma(struct wctdm *wc)
{
	...
	outb(0x01, wc->ioaddr + WC_CNTL);
	outb(0x01, wc->ioaddr + WC_OPER);
	__wctdm_setcreg(wc, WC_DMACTL, 0x01);		// test by ly
}

static void wctdm_stop_dma(struct wctdm *wc)
{
	outb(0x00, wc->ioaddr + WC_OPER);
	__wctdm_setcreg(wc, WC_DMACTL, 0x00);		// test 2012.11.20
}
...
static int __devinit wctdm_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	int x;
	int y;
	unsigned char ver;  // test 2012.11.20
	...
	if (pci_enable_device(pdev)) {
		...
		if (wc) {
			...
			outb(0x01, wc->ioaddr + WC_CNTL); //test by ly
			/* Check Freshmaker chip */
			x=inb(wc->ioaddr + WC_CNTL);
			ver = __wctdm_getcreg(wc, WC_VER);
			printk("Hardware_Version = 0x%x\n",ver); // test by ly
			wc->HW_VER = ver;

			/* Allocate enough memory for two zt chunks, receive and transmit.  Each sample uses
			   32 bits.  Allocate an extra set just for control too */
			if(wc->HW_VER > 0xa6)
				wc->writechunk = pci_alloc_consistent(pdev, DAHDI_MAX_CHUNKSIZE * 2 * 2 * 4, &wc->writedma); //512bytes
			else	
			...
			if(wc->HW_VER == 0xa7)
				{
					wc->readchunk = wc->writechunk + 64;	/* in doublewords */
					wc->readdma = wc->writedma + 64*4;		/* in bytes */
				}
			else if(wc->HW_VER == 0xa8)
				{
					wc->readchunk = wc->writechunk + 64;	/* in doublewords */
			...
			else if(wc->HW_VER < 0xa7)
				{
					...
					wc->readchunk = wc->writechunk + DAHDI_MAX_CHUNKSIZE * 2*((NUM_CARDS+NUM_SYNC)/4);	/* in doublewords */
					wc->readdma = wc->writedma + DAHDI_MAX_CHUNKSIZE * 8*((NUM_CARDS+NUM_SYNC)/4);		/* in bytes */
				}
			...
			if (wctdm_initialize(wc)) {
				...
				if (wc->freeregion)
					release_region(wc->ioaddr, 0xff);
				if(wc->HW_VER > 0xa6)
					pci_free_consistent(pdev, 512, (void *)wc->writechunk, wc->writedma);  //in byte
				else
				//pci_free_consistent(pdev, DAHDI_MAX_CHUNKSIZE * 2 * 2 * 2 * 4, (void *)wc->writechunk, wc->writedma);
				pci_free_consistent(pdev, DAHDI_MAX_CHUNKSIZE * 2 * 2 * (NUM_CARDS+NUM_SYNC), (void *)wc->writechunk, wc->writedma);
				kfree(wc);
				return -EIO;
			}
			...
			//if (request_irq(pdev->irq, wctdm_interrupt, IRQF_SHARED, "wctdm", wc)) {
			if (request_irq(pdev->irq, wctdm_interrupt, IRQF_SHARED, "ax1600p", wc)) {
				//printk(KERN_NOTICE "wctdm: Unable to request IRQ %d\n", pdev->irq);
				printk(KERN_NOTICE "ax1600p: Unable to request IRQ %d\n", pdev->irq);
				if (wc->freeregion)
					release_region(wc->ioaddr, 0xff);
				if(wc->HW_VER > 0xa6)
					pci_free_consistent(pdev, 512, (void *)wc->writechunk, wc->writedma);  //in byte
			...
			if (wctdm_hardware_init(wc)) {
				unsigned char x;  // test 2012.11.20
				/* Set Reset Low */
				x=inb(wc->ioaddr + WC_CNTL);
				outb((~0x1)&x, wc->ioaddr + WC_CNTL);
				/* Free Resources */
			...
			if(wc->HW_VER < 0xa7)
				memset((void *)wc->writechunk,0,DAHDI_MAX_CHUNKSIZE * 2 * 2 *(NUM_CARDS+NUM_SYNC));
			else
			...
			if(wc->HW_VER == 0xa8)
				printk("Found an Analog Card: ATCOM AX800P(E) (%d modules)\n",  cardcount);
			else
...
static DEFINE_PCI_DEVICE_TABLE(wctdm_pci_tbl) = {
	{ 0xe159, 0x0001, 0xb500, PCI_ANY_ID, 0, 0, (unsigned long) &wctdm },
    	{ 0xe159, 0x0001, 0xb700, PCI_ANY_ID, 0, 0, (unsigned long) &wctdm },
    	{ 0xe159, 0x0001, 0xb300, PCI_ANY_ID, 0, 0, (unsigned long) &wctdm }, 
#ifdef TDM_REVH_MATCHALL
	{ 0xe159, 0x0001, PCI_ANY_ID, PCI_ANY_ID, 0, 0, (unsigned long) &wctdm },
#endif
	{ 0 }
};
...
static struct pci_driver wctdm_driver = {
	...
	.suspend = NULL,	//test, 2012-11-20
	.resume = NULL,		//test, 2013-0402
...
MODULE_DESCRIPTION("ATCOM AX1600P Driver");
