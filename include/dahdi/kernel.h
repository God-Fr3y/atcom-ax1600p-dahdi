...
typedef struct __kernel_old_timespec	dahdi_timespec_t;
typedef struct __kernel_old_timeval	dahdi_timeval_t;
#else
typedef struct timespec			dahdi_timespec_t;
typedef struct timeval			dahdi_timeval_t;
#endif
...
/*
 * krefs were switched from using atomic_t to refcount_t in 4.11. This
 * version also added kref_read() to avoid needing to access ->refcount
 * directly, so use kref_read() in the dahdi drivers and provide a
 * compatible implementation using atomic_t for < 4.11.
 */
...
struct dahdi_device {
	...
	dahdi_timespec_t registration_time;
};
...
static inline void do_gettimeofday(dahdi_timeval_t *tv)
{
