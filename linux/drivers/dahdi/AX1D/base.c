...
/*
 * ATCOM AX1D/AXE1D Driver version 1.1
 *
...
static int t1e1override = -1;  /* 0xff for E1, 0x00 for T1, deprecated */
...
#define FLAG_EHALF   (1 << 11)
#define FLAG_MAX_BUF (1 << 31)		/*  gen 5 use fixed pre-alloced buffer */
...
static struct devtype AX1D = { "ATCOM AX1D/AXE1D ", FLAG_2NDGEN};	//test 2012.11.15
static struct devtype AX1DP5 = { "ATCOM HALF AXE1D ", FLAG_2NDGEN | FLAG_5THGEN | FLAG_EHALF};
...
struct t4_span {
	...
	int loopdowncnt;
#ifdef SUPPORT_GEN1
	unsigned char ec_chunk1[32][DAHDI_CHUNKSIZE]; /* first EC chunk buffer */
	unsigned char ec_chunk2[32][DAHDI_CHUNKSIZE]; /* second EC chunk buffer */
#endif
...
static void __set_clear(struct t4 *wc, int span)
{
	...
	if ((ts->linemode == T1) || (ts->linemode == J1)) {
		for (i=0;i<24;i++) {
			j = (i/8);
		...
		for (i=0;i<(unchannelized ? 32 : 31);i++) {
			if (ts->span.chans[i]->flags & DAHDI_FLAG_CLEAR)
				ts->notclear &= ~(1 << i);
			else 
				ts->notclear |= (1 << i);
...
static int t4_rbsbits(struct dahdi_chan *chan, int bits)
{
	...
	spin_lock_irqsave(&wc->reglock, flags);	
	k = chan->span->offset;
	if (ts->linemode == E1) { /* do it E1 way */
		if (chan->chanpos == 16) {
			spin_unlock_irqrestore(&wc->reglock, flags);
			return 0;
		}
...
static int t4_shutdown(struct dahdi_span *span)
{
	...
	tspan = span->offset + 1;
	if (tspan < 0) {
		dev_notice(&wc->dev->dev, "Span '%d' isn't us?\n",
				span->spanno);
		return -1;
	}
	...
	if ((wc->numspans == 1) && 
		...
		dev_info(&wc->dev->dev, "Disabling interrupts since "
	      ...
	      struct dahdi_lineconfig *lc)
{
	...
	if (debug & DEBUG_MAIN)
		dev_notice(&wc->dev->dev, "Configuring span %d\n",
				span->spanno);
...
t4_chanconfig(struct file *file, struct dahdi_chan *chan, int sigtype)
{
	...
	if (debug & DEBUG_MAIN) {
		if (alreadyrunning)
			dev_notice(&wc->dev->dev, "Reconfigured "
				"channel %d (%s) sigtype %d\n",
				chan->channo, chan->name, sigtype);
		else
			dev_notice(&wc->dev->dev, "Configured channel"
				" %d (%s) sigtype %d\n",
				chan->channo, chan->name, sigtype);
	}
...
static void init_spans(struct t4 *wc)
{
	...
	for (x = 0; x < wc->numspans; x++) {
	  char *spanname;
	//spanname = (wc->devtype->flags & FLAG_5THGEN) ? "D130" : "D115";
	//spanname = (wc->devtype->flags & FLAG_5THGEN) ? "D130" : "AX1D";
	//if (!(wc->devtype->flags & FLAG_5THGEN)) 	//test, 2012.11.19
	//	spanname = "AX1D"; 
                spanname = (wc->devtype->flags & FLAG_EHALF) ? "AXHE1D" : "AX1D";
		ts = wc->tspans[x];
		sprintf(ts->span.name, "%s/%d/%d", spanname, wc->num, x + 1);
		snprintf(ts->span.desc, sizeof(ts->span.desc) - 1,
			 "%s (E1|T1) Card %d Span %d", spanname, wc->num, x+1);
		switch (ts->linemode) {
		case T1:
			ts->span.spantype = "T1";
			break;
		...
		if (ts->linemode == T1 || ts->linemode == J1) {
			...
			ts->span.channels = (unchannelized ? 32 : 31);
...
/*		init_waitqueue_head(&ts->span.maintq); */
		...
		for (y=0;y<wc->tspans[x]->span.channels;y++) {
	   	...
	   	//if (!(wc->devtype->flags & FLAG_5THGEN)) 	//test, 2012.11.19
	    	//	spanname = "AX1D"; 
                spanname = (wc->devtype->flags & FLAG_EHALF) ? "AXHE1D" : "AX1D";
			sprintf(mychans->name, "%s/%d/%d/%d", spanname, wc->num, x + 1, y + 1);
...
static void t4_serial_setup(struct t4 *wc)
{
  ...
  if (debug)
			dev_info(&wc->dev->dev, "Setting up global serial parameters\n");
...
static int __t4_findsync(struct t4 *wc)
{
	...
	int newsyncnum = 0;			/* AX1D card number */	//test, 2012.11.15
	int newsyncspan = 0;		/* span on given AX1D card */	//test, 2012.11.15
...
static void __t4_configure_e1(struct t4 *wc, int unit, int lineconfig)
{
	...
	if (unchannelized)
		__t4_framer_out(wc, unit, 0x1f, 0x40);
	...
	if (unchannelized)
		__t4_framer_out(wc, unit, 0x21, 0x3c);
	else
	...
	dev_info(&wc->dev->dev, "Span %d configured for %s/%s%s\n",
...
static int t4_startup(struct file *file, struct dahdi_span *span)
{
	...
	tspan = span->offset + 1;
	if (tspan < 0) {
		dev_info(&wc->dev->dev, "Span '%d' isn't us?\n",
				span->spanno);
		return -1;
	}
	...
	if (ts->linemode == E1) { /* if this is an E1 card */
...
static void t4_receiveprep(struct t4 *wc, int irq)
{
	...
	if (dbl) {
		if (debug & DEBUG_MAIN)
			dev_notice(&wc->dev->dev, "Double/missed "
				"interrupt detected\n");
	}
...
static void t4_check_alarms(struct t4 *wc, int span)
{
	...
	if (c & 0x10) { /* receiving yellow (RAI) */
		if (ts->yelalarmcount >= yelalarmdebounce && !unchannelized)
		...
		if (debug & DEBUG_MAIN) {
			...
			if (isr3 & 0x01)
				dev_notice(&wc->dev->dev, "RECEIVE "
					...
					span + 1);
			if (isr4 & 0x80)
				dev_notice(&wc->dev->dev, "TRANSMIT "
					"slip POSITIVE on span %d\n",
					span + 1);
			if (isr4 & 0x40)
				dev_notice(&wc->dev->dev, "TRANSMIT "
...
static void t4_increase_latency(struct t4 *wc, int newlatency)
{
	...
	/* buffer already be maxim? */
	if(wc->flags & FLAG_MAX_BUF) {
		spin_lock_irqsave(&wc->reglock, flags);
		...
		spin_unlock_irqrestore(&wc->reglock, flags);
		dev_info(&wc->dev->dev, "Increased latency to %d w/o realloc\n", newlatency);
		return;
	}
...
static int __devinit t4_launch(struct t4 *wc)
{
	...
	if(debug) {
	  dev_info(&wc->dev->dev, "Launching card: %d\n",
			wc->order);
	}

  wc->ddev->manufacturer = "ATCOM";  //test, 2012.11.19
  ...
  list_add_tail(&wc->tspans[0]->span.device_node, &wc->ddev->spans);
			     ...
			     enum linemode linemode)
{
	...
	for (i = 0; i < ((E1 == ts->linemode) ? (unchannelized ? 32 : 31) : 24); i++) {
...
/**
 * AX1D_sort_cards - Sort the cards in card array by rotary switch settings.
 *
 */
static void AX1D_sort_cards(void)	//test, 2012.11.19
{
	int x;
...
static int __devinit t4_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	if (!(wc = kzalloc(sizeof(*wc), GFP_KERNEL))) {
		return -ENOMEM;
	}
  ...
  wc->devtype = (const struct devtype *)(ent->driver_data);
	...
	if (!request_mem_region(wc->memaddr, wc->memlen, wc->variety))
		dev_info(&wc->dev->dev, "Unable to request memory "
				"region :(, using anyway...\n");
#endif
	if (pci_request_regions(pdev, wc->variety))
		dev_info(&pdev->dev, "Unable to request regions\n");
	
	dev_info(&pdev->dev, "Found %s at base address %08lx, remapped "
			"to %p\n", wc->variety, wc->memaddr, wc->membase);
	...
	/* Initialize hardware */
	res = t4_hardware_init_1(wc, wc->flags);
	if (res) {
	...
	if (wc->flags & FLAG_2NDGEN) {
		...
		sprintf(tmp, "AX1D");		//test, 2012.11.19
	...
	for (x = 0; x < PORTS_PER_FRAMER; x++) {
		...
		if (wc->t1e1 & (1 << x)) {
			wc->tspans[x]->linemode = E1;
		} else {
			if (j1mode)
				wc->tspans[x]->linemode = J1;
			else
				wc->tspans[x]->linemode = T1;
		}

		for (f = 0; f < (wc->tspans[x]->linemode == E1 ? (unchannelized ? 32 : 31) : 24); f++) {
  ...
  /* Continue hardware intiialization */
...
#ifdef SUPPORT_GEN1
	if (request_irq(pdev->irq, (wc->flags & FLAG_2NDGEN) ? t4_interrupt_gen2 :t4_interrupt, DAHDI_IRQ_SHARED_DISABLED, "AX1D", wc))		//test, 2012.11.19 
#else
		if (!(wc->tspans[0]->spanflags & FLAG_2NDGEN)) {
			...
			if (request_irq(pdev->irq, t4_interrupt_gen2, DAHDI_IRQ_SHARED_DISABLED, "AX1D", wc))	//test,2012.11.19
#endif
	{
	...
	if (!ignore_rotary)
		AX1D_sort_cards();	//test, 2012.11.19

	dev_info(&wc->dev->dev, "Found an Atcom Card: %s\n", wc->variety);
	wc->gpio = 0x00000000;
...
static int t4_hardware_stop(struct t4 *wc)
{
	...
	dev_notice(&wc->dev->dev, "Stopped %s, Turned off DMA\n", wc->variety);
	return 0;
}
...
static void _t4_remove_one(struct t4 *wc)
{
  ...
  dahdi_unregister_device(wc->ddev);
	...
	/* ATCOM delay 1 seconds for pending dma end up */
	if(wc->flags & FLAG_MAX_BUF)
		mdelay(1000);	
	...
	pci_free_consistent(wc->dev, T4_BASE_SIZE * wc->numbufs * 2, (void *)wc->writechunk, wc->writedma);
...
{
	{ 0x1b74, 0x0115, PCI_ANY_ID, PCI_ANY_ID, 0, 0, (unsigned long)&AX1D }, /* ATCOM AX1D/AXE1D */  //test,2012.11.19
	{ 0x1b74, 0xd130, PCI_ANY_ID, PCI_ANY_ID, 0, 0, (unsigned long)&AX1DP5 }, /* ATCOM AX1D/AXE1D/AXE1DL */
	{ 0, }
};
...
static int __init t4_init(void)
{
	...
	if (cards[0] && cards[0]->order != 0) {
		printk(KERN_NOTICE "AX1D: Ident of first card is not zero (%d)\n",  //test,2012.11.19
			cards[0]->order);
	}
...
MODULE_DESCRIPTION("Unified ATCOM Single-port Digital Card Driver");  //test, 2012.11.19
MODULE_ALIAS("AX1D");		//test, 2012.11.19
